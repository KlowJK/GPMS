...
import { type OrgClass, listMajors } from '@/features/assistants/services/organization/orgApi';
...
type Props = {
  initial?: OrgClass;
  onClose: () => void;
  onSubmit: (data: { tenLop: string; nganhId: Id }) => Promise<any>; // 👈 chỉ payload
};
...
export default function ClassFormModal({ initial, onClose, onSubmit }: Props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await listMajors({ page: 0, size: 999 });
  ...
  async function handleSubmit() {
    ...
    if (!tenLop.trim() || !nganhId) {
      setErr('Vui lòng nhập tên lớp và chọn ngành.');
      return;
    }
    try {
      setLoading(true);
      await onSubmit({
        tenLop: tenLop.trim(),
        nganhId: /^\d+$/.test(nganhId) ? Number(nganhId) : (nganhId as unknown as Id),
      });
      // ❌ KHÔNG gọi onClose() ở đây; parent sẽ đóng sau khi save + reload
    } catch (e: any) {
    ...
    <div className="fixed inset-0 bg-black/40 grid place-items-center z-50">
      <div className="bg-white rounded-2xl p-6 w-[560px]">
        ...
        <div className="flex justify-end gap-3 mt-6">
          <button type="button" onClick={onClose} className="px-4 h-10 rounded bg-slate-200" disabled={loading}>
            Quay lại
          </button>
          <button
            type="button"                                // 👈 tránh submit ngầm
