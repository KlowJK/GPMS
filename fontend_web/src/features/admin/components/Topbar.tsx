...
type Props = {
  /** Mở sidebar trên mobile (nếu bạn muốn) */
  onOpenSidebar?: () => void;
  /** Thông tin người dùng (nếu có) */
  userName?: string;
  userEmail?: string;
  /** Callback đăng xuất (nếu có) */
  onLogout?: () => void;
};
...
export default function Topbar({
  ...
  return (
    <header className="relative h-20 w-full bg-white border-b shadow-sm flex items-center justify-between px-4 sm:px-6 rounded-b-xl">
      {/* Trái: nút mở menu (mobile) + logo + tiêu đề */}
            ...
            <div className="absolute right-0 mt-2 w-60 bg-white rounded-xl shadow-md overflow-hidden z-50">
              ...
              <div className="p-2">
                <button
                  ...
                  onClick={() => {
                    // Điều hướng đổi mật khẩu ở đây nếu cần
                    setOpenMenu(false);
                  }}
                >
                  <Key size={16} />
