...
class _NhatKyState extends State<NhatKy> {
  ...
  Future<void> _openSubmitPage({int? defaultWeek, int? deTaiId, int? idNhatKy, DateTime? ngayBatDau, DateTime? ngayKetThuc}) async {
    ...
    if (deTaiId != null || idNhatKy != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Đã nộp nhật ký thành công')));
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider<NhatKyViewModel>(
      ...
      create: (_) => NhatKyViewModel()..fetchTuans(includeAll: false)..fetchDiaries(includeAll: false),
      child: Consumer<NhatKyViewModel>(builder: (context, vm, _) {
        if (vm.error != null) {
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    if (_items.isEmpty && vm.diaries.isEmpty)
                      const _EmptyState(icon: Icons.edit_note, title: 'Bạn chưa có nhật ký trong hệ thống.', subtitle: 'Nhấn nút “+” để nộp nhật ký tuần.')
                    else
                    ...
                    Card(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Nhật ký ', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600)),
                            const SizedBox(height: 8),
  ...
  (Color bg, Color fg, String label) get _badge {
    switch (item.status) {
      case DiaryStatus.approved:
        return (const Color(0xFFDCFCE7), const Color(0xFF166534), 'GVHD đã xác nhận');
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text('Tuần ${item.week}', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600))),
...
class _DiaryItemCard extends StatelessWidget {
  ...
  _Badge _statusBadge(String? raw) {
    ...
    if (s.contains('chưa') || s.contains('chua') || s.contains('chua nop') || s.contains('chua nộp')) {
      return const _Badge(text: 'Chưa nộp', bg: Color(0xFFFFF4D6), fg: Color(0xFF7A4B00));
    }
    if (s.contains('đã') || s.contains('da') || s.contains('đã nộp') || s.contains('da nop') || s.contains('nop')) {
      return const _Badge(text: 'Đã nộp', bg: Color(0xFFDFF7E7), fg: Color(0xFF10603A));
    }
    // fallback: show original text in neutral badge
    return _Badge(text: raw ?? '-', bg: const Color(0xFFF1F5F9), fg: Colors.black54);
  }

  // Render file meta as a shortened clickable link
  Widget _fileRow(BuildContext context, String url) {
    final uri = Uri.tryParse(url);
    final short = (uri != null && uri.pathSegments.isNotEmpty) ? uri.pathSegments.last : url;

    return Padding(
      padding: const EdgeInsets.only(top: 6),
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        SizedBox(width: 110, child: const Text('File', style: TextStyle(color: Colors.black54))),
        const SizedBox(width: 8),
        Expanded(
          child: InkWell(
            onTap: () async {
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Expanded(child: Text(short, style: const TextStyle(color: Color(0xFF2563EB)), maxLines: 1, overflow: TextOverflow.ellipsis)),
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text('Tuần ${item.tuan ?? '-'}', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600))),
            const SizedBox(width: 8),
          ...
          _meta('Thời gian', '${_fmt(item.ngayBatDau)} – ${_fmt(item.ngayKetThuc)}'),
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: [Icon(icon, size: 56, color: cs.primary), const SizedBox(height: 12), Text(title, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600), textAlign: TextAlign.center), const SizedBox(height: 6), Text(subtitle, textAlign: TextAlign.center)]),
    );
  }
}
...
class _Badge extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return DecoratedBox(decoration: ShapeDecoration(color: bg, shape: const StadiumBorder()), child: Padding(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), child: Text(text, style: TextStyle(color: fg, fontSize: 12))));
  }
}
